...
process.env.NEWSGRAPH_CURSOR_SECRET = "fixture-only-cursor-secret";
...
globalThis.fetch = async (input, init) => {
    ...
    if (url.origin === "http://newsgraph.test")
...
try {
    process.env.NEWSGRAPH_API_BASE_URL = "http://newsgraph.test";
    rows = Array.from({ length: 12 }, (_, i) => article(i + 1));
    await import("./verify-integrations.mjs");
    const { NewsGraphClient } = await import("../packages/sdk/dist/index.js");
    const client = new NewsGraphClient({ baseUrl: "http://newsgraph.test" });
    ...
    const initial = await (await fetch(`http://newsgraph.test/v2/news?beat_id=${beat}`)).json();
    const history = await (await fetch(`http://newsgraph.test/v2/news?beat_id=${beat}&before=${encodeURIComponent(initial.history_cursor)}`)).json();
    assert(BigInt(history.items[0].id) < BigInt(initial.items.at(-1).id));
    const misuse = await fetch(`http://newsgraph.test/v2/changes?beat_id=${beat}&cursor=${encodeURIComponent(initial.history_cursor)}`);
    ...
    const failed = await fetch("http://newsgraph.test/v2/news?beat_id=nonexistent");
