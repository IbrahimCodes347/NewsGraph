...
export async function executeNewsTool(baseUrl, call) {
    ...
    const paths = { newsgraph_topics: "topics", newsgraph_news: "news", newsgraph_changes: "changes" };
    if (!Object.hasOwn(paths, name))
        throw new Error(`Unknown news tool: ${name}`);
    if (name !== "newsgraph_topics" && typeof args.beat_id !== "string")
        throw new Error("beat_id is required");
    if (name === "newsgraph_changes" && typeof args.cursor !== "string")
        throw new Error("cursor is required");
    const query = new URLSearchParams(name === "newsgraph_topics" ? { q: args.query ?? "" } : { beat_id: args.beat_id, ...(args.cursor ? { cursor: args.cursor } : {}) });
