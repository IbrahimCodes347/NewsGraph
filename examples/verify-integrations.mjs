...
import { NewsGraphClient } from "../packages/sdk/dist/index.js";
import { loadNewsTools, executeNewsTool } from "./openrouter-tools.mjs";
const base = process.env.NEWSGRAPH_API_BASE_URL ?? "http://127.0.0.1:8787";
...
if (process.env.NEWSGRAPH_API_TOKEN) {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (input, init) => {
        const request = new Request(input, init);
        if (request.url.startsWith(`${base}/`)) {
            request.headers.set("Authorization", `Bearer ${process.env.NEWSGRAPH_API_TOKEN}`);
        }
        return originalFetch(request);
    };
}
const sdk = new NewsGraphClient({ baseUrl: base });
...
const result = await executeNewsTool(base, { id: "verification-call", function: { name: "newsgraph_news", arguments: JSON.stringify({ beat_id: beat }) } });
...
const client = new Client({ name: "newsgraph-verification", version: "1.0.0" });
try {
    await client.connect(new StreamableHTTPClientTransport(new URL(`${base}/mcp`)));
    const listed = await client.listTools();
    assert.equal(listed.tools.length, 3);
    const result = await client.callTool({ name: "newsgraph_news", arguments: { beat_id: beat } });
