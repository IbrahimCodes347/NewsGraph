# NewsGraph — Supabase go-live runbook
...
`@newsgraph/sdk` clients point `baseUrl` at `https://<REF>.supabase.co/functions/v1/api`.
...
| Realtime broadcast on packs | live (publication enabled by migration 0003) — consume via `@newsgraph/realtime` |
...
curl -X POST "https://api.supabase.com/v1/projects/<REF>/custom-hostname/initialize" \
  ...
  -H "Content-Type: application/json" \
  -d '{"custom_hostname":"api.newsgraph.vercel.app"}'
```
...
4. The API is then served at `https://api.newsgraph.vercel.app/functions/v1/api/…`;
   point `@newsgraph/sdk` `baseUrl` at `https://api.newsgraph.vercel.app/functions/v1/api`.
